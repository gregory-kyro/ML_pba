...
def create_dataset(PDBbind_dataset_path, general_set_PDBs_path, refined_set_PDBs_path, plot = False):
  ...
  affinity_data = data[['pdbid','-logKd/Ki']]
  affinity_data.to_csv('affinity_data.csv')
  
  # check for NaNs in affinity data
  if affinity_data['-logKd/Ki'].isnull().any() != False:
    print('There are NaNs present in affinity data!')
    
  # create list of PDB id's in affinity_data
  pdbid_list = list(affinity_data['pdbid'])
  ...
  # add 'set' column to affinity_data and fill with 'general'/'refined'
  affinity_data['set'] = np.nan
  affinity_data.loc[np.in1d(affinity_data['pdbid'], list(general_dict)), 'set'] = 'general'
  affinity_data.loc[np.in1d(affinity_data['pdbid'], list(refined_dict)), 'set'] = 'refined'
  ...
  display(pd.read_csv('affinity_data_cleaned.csv'))
  
  if plot == True:
    # plot affinity distributions for general and refined sets
    grid = sns.FacetGrid(affinity_data, row='set', row_order=['general', 'refined'],
                     size=3, aspect=2)
